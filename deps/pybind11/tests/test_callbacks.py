...
def test_keyword_args_and_generalized_unpacking():
    ...
    if test_tuple_unpacking(f) != (("positional", 1, 2, 3, 4, 5, 6), {}):
        raise AssertionError
    if test_dict_unpacking(f) != (("positional", 1), {"key": "value", "a": 1, "b": 2}):
        raise AssertionError
    if test_keyword_args(f) != ((), {"x": 10, "y": 20}):
        raise AssertionError
    if test_unpacking_and_keywords1(f) != ((1, 2), {"c": 3, "d": 4}):
        raise AssertionError
    if test_unpacking_and_keywords2(f) != (
    ...
    if "Got multiple values for keyword argument" not in str(excinfo.value):
        raise AssertionError
    ...
    if "Unable to convert call argument" not in str(excinfo.value):
        raise AssertionError
...
def test_cpp_function_roundtrip():
    ...
    if test_dummy_function(dummy_function) != "matches dummy_function: eval(1) = 2":
        raise AssertionError
    if test_dummy_function(roundtrip(dummy_function)) != "matches dummy_function: eval(1) = 2":
        raise AssertionError
    if roundtrip(None, expect_none=True) is not None:
        raise AssertionError
    if test_dummy_function(lambda x: x + 2) != "can't convert to function pointer: eval(1) = 3":
        raise AssertionError
    ...
    if not any(s in str(excinfo.value) for s in ("missing 1 required positional argument",
...
def test_function_signatures(doc):
    ...
    if doc(test_callback3) != "test_callback3(arg0: Callable[[int], int]) -> str":
        raise AssertionError
    if doc(test_callback4) != "test_callback4() -> Callable[[int], int]":
        raise AssertionError
