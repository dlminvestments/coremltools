  ...
  class MessageContainerTest < Test::Unit::TestCase
    ...
    def test_ctor_args
      m = TestMessage.new(optional_int32: -42,
                          ...
                          optional_enum: :C,
                          repeated_string: %w[hello there world])
      assert m.optional_int32 == -42
      assert m.optional_msg.instance_of?(TestMessage2)
    ...
    def test_rptfield_int32
      ...
      assert m.repeated_int32.equal?(l)
      l.push 42
    ...
    def test_parent_rptfield
      ...
      assert m.repeated_string == %w[ok ok2]
      m.repeated_string += ['ok3']
      assert m.repeated_string == %w[ok ok2 ok3]
    end
    ...
    def test_map_dup_deep_copy
      ...
      assert m['a'].equal?(m2['a'])
      assert m['b'].equal?(m2['b'])
    ...
    def test_map_field
      ...
      assert m.map_string_int32.keys.sort == %w[a b]
    ...
    def test_oneof_descriptors
      ...
      assert !o.nil?
      assert o.instance_of?(Google::Protobuf::OneofDescriptor)
      assert o.name == 'my_oneof'
      oneof_count = 0
      d.each_oneof  do |oneof|
        oneof_count += 1
        assert oneof == o
      end
    ...
    def test_dup
      ...
      assert m.repeated_msg[0].equal?(m2.repeated_msg[0])
    end
    ...
    def test_encode_decode_helpers
      m = TestMessage.new(optional_string: 'foo', repeated_string: %w[bar1 bar2])
      assert_equal 'foo', m.optional_string
      assert_equal %w[bar1 bar2], m.repeated_string

      json = m.to_json
      m2 = TestMessage.decode_json(json)
      assert_equal 'foo', m2.optional_string
      assert_equal %w[bar1 bar2], m2.repeated_string
    ...
    def test_reflection
      ...
      assert msgdef.instance_of?(Google::Protobuf::Descriptor)
      assert msgdef.any? { |field| field.name == 'optional_int32' }
      optional_int32 = msgdef.lookup 'optional_int32'
      assert optional_int32.instance_of?(Google::Protobuf::FieldDescriptor)
      ...
      assert optional_enum.subtype.instance_of?(Google::Protobuf::EnumDescriptor)
